...
export const fetchModeBackendVersionAPI = async () => {
  const request = useRequest()

  const { mode } = await request.get('version').json<BackendVersion>()

  return mode
}

export const fetchPrimitiveAPI = async () => {
  const request = useRequest()

  return await request.get('primitive').json<ProfileData>()
}

export const updatePrimitiveAPI = async (data: ProfileData) => {
  const request = useRequest()

  const resp = await request.post('primitive', { json: data })
